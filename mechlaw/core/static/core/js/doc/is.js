$(document).ready(function() {
    ...
    $('changed-by changing-law').each(function() {
        ...
        $changing_law.find('changed-portion').each(function() {
            ...
            // Start with the entire law and narrow it down later. The
            // variables $start_mark and $end_mark denote the highlight's
            // start point and end point, respectively. In the beginning, they
            // are both set to the entire legal document's XML, but then
            // narrowed down afterwards. This means that each node in
            // <changed-portion> narrows it down by one step, until eventually
            // we have the final node, which is the one where the highlighting
            // should take place.
            //
            // Usually, the $start_mark and $end_mark end
            // up being the same thing. They are only different when the
            // highlight covers a range of entities.
            ...
            $portion.children().each(function() {
                ...
                // Check if the value is a range, for example "1-4", which
